...
else
{
  ...
  */
  ...
  {
		setcookie("lastName", $info[0]["sn"][0], time()+3600, "/~recruit", "bm.ust.hk");
		setcookie("firstName", $info[0]["givenname"][0], time()+3600, "/~recruit", "bm.ust.hk");
		setcookie("mail", $info[0]["mail"][0], time()+3600, "/~recruit", "bm.ust.hk");
	  setcookie("isust", "is-".MD5(TIME()), time()+3600, "/~recruit", "bm.ust.hk");
		setcookie("lastName");
		setcookie("firstName");
		setcookie("mail");
	  setcookie("isust");

    if ($HTTP_COOKIE_VARS["lastName"] == "")
      warning("Cookies cannot be set in your browser. Please use another computer or browser."
              ." Necscape 4.x or 6 is recommanded");
    else
