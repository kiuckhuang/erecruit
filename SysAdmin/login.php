...
if(isset($PHP_AUTH_USER)  && $PHP_AUTH_USER != "" && $PHP_AUTH_PW != "")  {
	...
	if($bind){
		...
		for ($i=0; $i<$info["count"]; $i++) {
			...
			$fields = "cn|departmentcode|givenname|objectclass|sn|uid|userclass|ou|mailid|mailhost|mailquota|maildrop|ustonlyattribute|mail|cn;lang-zh-tw|mobile|labeleduri|modifiersname|modifytimestamp";

			$data = explode("|", $fields);
			for($j=0; $j <18; $j++){
				$field = $data[$j];
				echo $field.": ".$info[$i]["$field"][0]."<br>";
			}
